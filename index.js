    ...
    raffx.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
               ...
               if (anu.action == 'add') {
                    raffx.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `Welcome anj @${num.split("@")[0]} To Group ${metadata.subject} 👋` })
                } else if (anu.action == 'remove') {
                    raffx.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `haha habis ngambil cewe maka nya keluar @${num.split("@")[0]} 👋` })
